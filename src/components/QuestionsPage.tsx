...
export default function QuestionsPage({ content }: QuestionsPageProps) {
  const parseMarkdown = (text: string) => {
    return text
      .replace(/^# (.+)$/gm, '<h1 class="text-2xl sm:text-3xl font-bold text-white mb-6">$1</h1>')
      ...
      .replace(/^- (.+)$/gm, '<li class="ml-4 text-gray-300 mb-2">• $1</li>')
      .replace(/^(\d+)\. (.+)$/gm, '<li class="ml-4 text-gray-300 mb-2">$1. $2</li>')
      ...
      .replace(/^---$/gm, '<hr class="my-6 border-white/20">')
      .replace(/\n\n/g, '<br/><br/>')
      .replace(/\n/g, '<br/>');
  };
    ...
    <div className="container mx-auto px-4 py-8 max-w-4xl">
      ...
      {content ? (
        <div className="bg-black/40 backdrop-blur-sm rounded-2xl p-6 sm:p-8 border border-white/10">
          <div 
            className="prose prose-lg max-w-none text-gray-300 leading-relaxed faq-content"
