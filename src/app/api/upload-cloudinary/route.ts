...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('🔧 Configuration Cloudinary LANATION:', {
    ...
    console.log('📁 Fichier reçu LANATION:', {
      name: file.name,
      type: file.type,
    ...
    if (!hasValidExtension) {
      console.log('⚠️ Type de fichier non reconnu LANATION:', { name: file.name, type: file.type, extension: fileExtension });
    ...
    console.log('☁️ Upload vers Cloudinary LANATION...');
    ...
    try {
      ...
      console.log('📋 Buffer créé LANATION:', buffer.length, 'bytes');
    } catch (error) {
      console.error('❌ Erreur création buffer LANATION:', error);
      throw new Error('Impossible de lire le fichier');
    }

    // Upload vers Cloudinary avec gestion d'erreur améliorée
    console.log('⚡ Début upload vers Cloudinary LANATION...');
    ...
    const uploadResult = await new Promise((resolve, reject) => {
      ...
      console.log('☁️ Options upload LANATION:', uploadOptions);

      const uploadStream = cloudinary.uploader.upload_stream(
        uploadOptions,
        (error, result) => {
          if (error) {
            console.error('❌ Erreur Cloudinary détaillée LANATION:', {
            ...
            console.log('✅ Upload Cloudinary réussi LANATION:', {
      ...
      if (!uploadStream) {
        console.error('❌ Impossible de créer le stream upload LANATION');
    ...
    console.log('🎯 Upload terminé LANATION:', {
      url: result.secure_url,
      type: response.type,
      size: Math.round(file.size / 1024 / 1024 * 100) / 100 + 'MB'
    });
    ...
    console.error('❌ Erreur upload Cloudinary LANATION:', error);
    return NextResponse.json({ 
